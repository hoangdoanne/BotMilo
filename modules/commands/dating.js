...
module.exports.handleReaction = async function({ api, event, Threads, Users, Currencies, handleReaction }) {
    ...
    switch (turn) {
        case "match":
            ...
            var msg = {
                body: `[💏] ${senderInfo.name} - Người mà hệ thống chọn cho bạn là: ${random.name}\n[💌] Phù hợp: ${Math.floor(Math.random() * (80 - 30) + 30)}%\n\nNếu cả 2 người cùng chấp nhận Dating, hãy cùng nhau thả cảm xúc trái tim [❤] vào tin nhắn này và chính thức trạng thái dating với nhau.`,
...
module.exports.run = async function({ api, event, args, Users, Currencies }) {
    ...
    switch (args[0]) {
        case "Nam":
        case "nam":
            if (senderInfo.data.dating && senderInfo.data.dating.status == true) return api.sendMessage(`Muốn cắm sừng người ta hay sao ?, hãy làm một con người có trách nhiệm nào. Bạn hiện đang ở trạng thái Dating rồi còn muốn kiếm thêm người khác à 😈`, threadID, messageID);
            type = "Nam";
            break;
        case "Nữ":
        case "nữ":
        ...
        case "breakup":
            ...
            if (Date.now() - userInfo.data.dating.time.origin > 604800000) return api.sendMessage(`Còn chưa tới 7 ngày mà muốn chia tay là sao? 🥺\n\n${msgBreakup()}\n\nHãy cứ bình tĩnh suy nghĩ, cho mọi chuyện dần lắng xuống rồi giải quyết cùng nhau nhé vì tình yêu không phải ai cũng may mắn tìm thây nhau mà ^^`, threadID, messageID);
            var userMates = await Users.getData(userInfo.data.dating.mates);
            return api.sendMessage(`Cả 2 người thật sự không thể tiếp tục được nữa hay sao ?\nNếu có xem thấy dòng tin nhắn này, hãy cứ chi mọi chuyện lắng xuống... Yên lặng một chút, suy nghĩ cho kĩ vào...\nCó nhiều thứ... Một khi đã mất đi thì sẽ không thể tìm lại được nữa. ^^\n\nCòn nếu...Vẫn không thể tiếp tục cùng nhau nữa... Cả 2 người hãy thả cảm xúc vào tin nhắn này nhé!`, threadID, (error, info) => {
        ...
        case "info":
            var userInfo = await Users.getData(senderID);
            if (!userInfo.data.dating || userInfo.data.dating && userInfo.data.dating.status == false) return api.sendMessage(`Bạn bị F.A sml thì xem info cái gì ?`, threadID, messageID);
    ...
    if (money < 2000) return api.sendMessage(`Bạn cần 2000$ phí mua nhẫn PNJ tặng người dating với bạn💸`, threadID, messageID);
    return api.sendMessage(`Bạn sẽ bị trừ 2000$ phí mua nhẫn PNJ tặng người dating với bạn\nSố tiền này sẽ không hoàn trả nếu 1 trong 2 người không chấp nhận tiến vào trạng thái Dating❤\n\nThả cảm xúc vào tin nhắn này nếu chấp nhận tìm kiếm một người.`, threadID, (error, info) => {
