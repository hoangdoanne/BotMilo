    ...
    if (body.indexOf("ready") == 0) {
        ...
        if (values.player.length == values.ready) {
            ...
            for (const info of player) {
                ...
                ranking.push(`${num++} • ${name}: ${info.card1.Icon}${info.card1.Value} 丨 ${info.card2.Icon}${info.card2.Value} 丨 ${info.card3.Icon}${info.card3.Value} => ${info.tong} điểm\n`);
            }
  ...
  var money = data.money     
    ...
    case 'check': {
      for (const key in values.player) {
        ...
        default: {
            console.log("[ BOT ] » Hi, have a good day.")
        }
    }
}
...
function getLinkCard(Value, Suit) {
  return `https://raw.githubusercontent.com/PhucCuTe/card/main/cards/${Value == "J" ? "jack" : Value == "Q" ? "queen" : Value == "K" ? "king" : Value == "A" ? "ace" : Value}_of_${Suit}.png`;
}
