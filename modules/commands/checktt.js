...
module.exports.run = async ({ args, api, event, Currencies }) => {
    ...
    if (args[0] == "all") {
        ...
        return api.sendMessage(`${infoUser.name} đứng hạng ${rank} với ${infoUser.exp} tin nhắn`, event.threadID, event.messageID);
    }
    else {
        let storage = [], exp = [];
        for (const value of data.userInfo) storage.push({"id" : value.id, "name": value.name});
        ...
        let infoUser = exp[rank - 1];
        return api.sendMessage(`Bạn đứng hạng ${rank} với ${infoUser.exp} tin nhắn`, event.threadID, event.messageID);
    }
}